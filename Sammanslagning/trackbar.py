...
*Consider changing how small rectangles we tolerate - line 323
*Consider increasing the iterations - Line 306
'''
...
#field = cv2.VideoCapture("ppl.mp4")
#field = cv2.VideoCapture("videogång.mov")
...
cv2.namedWindow("X")
cv2.createTrackbar("Lower", "X", 20,50,nothing)
cv2.createTrackbar("Higher", "X", 255,255,nothing)
# Mainloop
while field.isOpened():
    ...
    low = cv2.getTrackbarPos("Lower", "X")
    high = cv2.getTrackbarPos("Higher", "X")
